...
echo ""
for t in integrity source-purity input-layer liveserver formulas masters components engine workbook-v1 freeze-check-v1 stale test routing handlers erp values tube verify bores inputs inquiry demo scenario persistence calcbar full; do
  [ -f "$t.js" ] || continue
  printf "  %-12s " "$t"
