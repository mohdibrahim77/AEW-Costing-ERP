...
echo ""
for t in integrity liveserver formulas masters components engine stale test routing handlers erp values tube verify bores inputs inquiry demo scenario persistence calcbar full; do
  [ -f "$t.js" ] || continue
  printf "  %-12s " "$t"
